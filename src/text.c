...
char *parse_text_header(char *buf, NIDS_text *t) {
	...
	t->num_chars = GET2(buf);
	t->x_start = GET2(buf + 2);
	t->x_start = GET2(buf + 4);
	
	if (!(t->chars = malloc(t->num_chars))) {
		fprintf(stderr, "ERROR: parse_text_header : %s\n", strerror(errno));
		exit(EXIT_FAILURE);
	}
	
	p = buf + 6;
	
	for (i = 0 ; i < t->num_chars - 4 ; i++, p++) {
		t->chars[i] = *p;
	}
	t->chars[i] = 0;
...
void print_text_header(NIDS_text *t, int ln) {
	...
	printf("data.symb.layers[%i].text.num_chars %i\n", ln, t->num_chars);
	printf("data.symb.layers[%i].text.x_start %i\n", ln, t->x_start);
	printf("data.symb.layers[%i].text.y_start %i\n", ln, t->y_start);
	printf("data.symb.layers[%i].text.chars %s\n", ln, t->chars);
